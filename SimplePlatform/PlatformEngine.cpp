...
void PlatformEngine::createScenarioIfNecessary (int refYear, const double* originLLA)
{
	// If the scenario is already initialized, no need to initialize again
	if (!PIData::isScenarioInitialized())
	{
		PIData::initializeScenario("Track Emulation",		// Generic file description
			9000,											// Port number: ignored
			PI_NETWORK_PROTOCOL_MULTICAST);					// Protocol: ignored
			//liveMode);										// Live mode flag
		PIData::setDescription ("Keyport RDM Plugin Demo");
		PIData::setClassification ("UNCLASSIFIED");
		PIData::setClassificationColor (PIRGBA (0, 255, 0, 128));
		PIData::setReferenceYear (refYear);
